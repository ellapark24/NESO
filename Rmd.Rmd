...
\begin{eqnarray*}
 M_F:  y_i = \beta_0 + \beta_1 \text{solar}_i + \beta_2 \text{wdaynumber}_i + \beta_3 \text{month}_i + \beta_4 \text{year}_i + \beta_5 \text{DSN}^2_i + \beta_6 \text{TE}_i  + \beta_7 \, \text{DSN_i}^2\text{:month_i} + \epsilon_i
\end{eqnarray*}
...
Using cross validation we find the following predictive scores for our final model as well as a basic model and more complex model:

```{r Other RMSE Table, eval=TRUE, echo=FALSE, warning=FALSE, message=FALSE}


cv.other.scores <- bind_rows(cv.scores.final, cv.scores.m1) %>%
                            mutate(Model = c("$M_F$", "$M_{B}$")) %>%
...
# Print the table
kable(cv.other.scores, caption = "Predictive Scores for Final and Baseline Model")
...
Secondly consider the observed vs predicted values from our cross validation  in Figure 2. While the points lie scattered around the 
